...
{
    private readonly ConcurrentDictionary<uint, List<KeyValuePair<int, long>>> _index = new();
    ...
    /// <summary>
    /// Checks if a Page/Version are in WAL-index memory. 
    /// Consider version that are below parameter. 
    /// Returns PagePosition of this page inside WAL-file or Empty if page doesn't found.
    /// Returns walVersion found (vs version request)
    /// </summary>
    ...
    /// </summary>
    public void ConfirmTransaction(uint transactionID, ICollection<PageLogLocation> pagePositions)
    {
    ...
    public void RestoreIndex(ref HeaderPage header)
    {
        ...
        //var positions = new Dictionary<long, List<PageLogLocation>>();
        //var current = 0L;

        //// read all pages to get confirmed transactions (do not read page content, only page header)
        //foreach (var buffer in _disk.(FileOrigin.Log))
        //{
        //    if(buffer.IsBlank())
        //    {
