...
internal class IncludeEnumerator : IPipeEnumerator
{
    ...
    private void DoInclude(BsonDocument value, PipeContext context)
    {
        ...
        {
            var (indexNode, _) = context.IndexService.Find(collection.PK, refId, false, Query.Ascending);

            if (!indexNode.IsEmpty)
            {
                var refDocResult = context.DataService.ReadDocument(indexNode.DataBlockID, Array.Empty<string>());
